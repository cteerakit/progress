...
const ICON_PATHS: Record<SlideStatus, string> = {
  none:
...
function createPath(status: SlideStatus): SVGPathElement {
  const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
  path.setAttribute('d', ICON_PATHS[status]);
...
const PROGRESS_RING_FILL_COLOR = '#34a853';
...
export function createProgressRing(
  ...
  progress.setAttribute('stroke', PROGRESS_RING_FILL_COLOR);
...
export function setProgressRing(svg: SVGSVGElement, percent: number): void {
